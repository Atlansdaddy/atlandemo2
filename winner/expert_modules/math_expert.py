...
class MathExpertModule(BaseExpertModule):
    ...
    def _analyze_mathematical_problem(self, query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        elif any(pattern in query_lower for pattern in ['calculate', 'compute', '+', '-', '*', '/', '%', 'percent']):
    ...
    def _solve_arithmetic(self, query: str, numbers: List[float]) -> Dict[str, Any]:
        """Solve basic arithmetic problems with proper expression evaluation."""
        steps = []
        formulas = []
        ...
        if expression:
            try:
                result = self._evaluate_expression(expression, steps)
                return {'answer': str(result), 'steps': steps, 'formulas': formulas}
            except:
                pass
        
        # Fallback to simple operations if expression parsing fails
    ...
    def _extract_math_expression(self, query: str) -> Optional[str]:
        ...
        percentage_match = re.search(r'(\d+(?:\.\d+)?)%\s*of\s*(\d+(?:\.\d+)?)', query, re.IGNORECASE)
        if percentage_match:
            return f"{percentage_match.group(1)}% of {percentage_match.group(2)}"
        
        # Look for expressions like "what is 2 + 3 * 4"
        match = re.search(r'(?:what is|calculate|solve|find)\s+([0-9+\-*/().\s%of]+)', query, re.IGNORECASE)
        if match:
        ...
        expr_match = re.search(r'([0-9+\-*/().\s%of]+)', query)
        if expr_match:
            expr = expr_match.group(1).strip()
            # Only return if it contains operators or percentage
            if any(op in expr for op in ['+', '-', '*', '/', '%']) or 'of' in expr:
                expr = expr.replace('×', '*').replace('÷', '/').replace('−', '-')
                return expr
        
        return None
    
    def _evaluate_expression(self, expression: str, steps: List[str]) -> float:
        """Evaluate mathematical expression with proper order of operations."""
        steps.append(f"Expression to evaluate: {expression}")
        
        # Handle percentages first
        if '%' in expression:
            # Convert percentages like "25% of 80" to "0.25 * 80"
            ...
            if percentage_match:
                ...
                steps.append(f"Convert percentage: {percentage_match.group(1)}% of {number} = {percent} × {number} = {result}")
                return result
        ...
        try:
            # Use eval for proper order of operations (safe for mathematical expressions)
            # First validate the expression contains only safe characters
            if re.match(r'^[0-9+\-*/().\s]+$', expression):
                result = eval(expression)
                steps.append(f"Following order of operations: {expression} = {result}")
                return float(result)
        except:
            pass
        
        # If eval fails, try manual parsing for simple cases
        return self._manual_evaluate(expression, steps)
    
    def _handle_fractions(self, expression: str, steps: List[str]) -> Optional[float]:
        """Handle fraction operations like 1/2 + 3/4."""
        # Look for fraction addition: a/b + c/d
        fraction_add = re.search(r'(\d+)/(\d+)\s*\+\s*(\d+)/(\d+)', expression)
        if fraction_add:
            ...
            g = gcd(result_num, common_denom)
            final_num = result_num // g
            final_denom = common_denom // g
            
            steps.append(f"Convert to common denominator: {a}/{b} + {c}/{d} = {new_a}/{common_denom} + {new_c}/{common_denom}")
            steps.append(f"Add numerators: {new_a} + {new_c} = {result_num}")
            if g > 1:
                steps.append(f"Simplify: {result_num}/{common_denom} = {final_num}/{final_denom}")
            
            return final_num / final_denom if final_denom != 1 else final_num
            
        return None
    
    def _manual_evaluate(self, expression: str, steps: List[str]) -> float:
        """Manual evaluation for simple expressions."""
        # Remove spaces
        expr = expression.replace(' ', '')
        
        # Handle simple multiplication and division first (left to right)
        while '*' in expr or '/' in expr:
            ...
            if mult_pos != -1 and (div_pos == -1 or mult_pos < div_pos):
                ...
                left_num, right_num, start, end = self._extract_operands(expr, mult_pos)
                result = left_num * right_num
                steps.append(f"Multiply: {left_num} × {right_num} = {result}")
                expr = expr[:start] + str(result) + expr[end:]
            elif div_pos != -1:
                # Handle division
                left_num, right_num, start, end = self._extract_operands(expr, div_pos)
                result = left_num / right_num if right_num != 0 else float('inf')
        ...
        while '+' in expr or '-' in expr[1:]:  # Skip first character for negative numbers
            # Find first + or -
            add_pos = expr.find('+')
            sub_pos = expr.find('-', 1)  # Skip first character
            
            if add_pos != -1 and (sub_pos == -1 or add_pos < sub_pos):
                # Handle addition
                left_num, right_num, start, end = self._extract_operands(expr, add_pos)
                result = left_num + right_num
                steps.append(f"Add: {left_num} + {right_num} = {result}")
                expr = expr[:start] + str(result) + expr[end:]
            elif sub_pos != -1:
                # Handle subtraction
                left_num, right_num, start, end = self._extract_operands(expr, sub_pos)
                result = left_num - right_num
    ...
    def _extract_operands(self, expr: str, op_pos: int) -> Tuple[float, float, int, int]:
        """Extract left and right operands around an operator position."""
        # Find left operand
        left_start = op_pos - 1
        while left_start > 0 and (expr[left_start - 1].isdigit() or expr[left_start - 1] == '.'):
            left_start -= 1
        left_num = float(expr[left_start:op_pos])
        
        # Find right operand
        right_end = op_pos + 1
        while right_end < len(expr) and (expr[right_end].isdigit() or expr[right_end] == '.'):
            right_end += 1
        right_num = float(expr[op_pos + 1:right_end])
    ...
    def _solve_equation(self, query: str, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Quadratic equation: x^2 = n
        quad_simple = re.search(r'x\s*\^\s*2\s*=\s*(\d+)', query)
        if quad_simple:
            n = float(quad_simple.group(1))
            if n >= 0:
                x = int(math.sqrt(n))
                steps.append(f"Quadratic: x² = {n}")
                steps.append(f"Take square root: x = ±√{n}")
                steps.append(f"Solutions: x = ±{x}")
                return {'answer': f'±{x}', 'steps': steps, 'formulas': ['x² = n → x = ±√n']}
            else:
                return {'answer': 'No real solutions', 'steps': ['Cannot take square root of negative number'], 'formulas': []}
